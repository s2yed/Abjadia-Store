    ...
    <div class="p-5 flex flex-col flex-grow">
        ...
        <div class="flex items-end justify-between mt-auto">
            <div class="flex flex-col">
                @if($product->discount_price)
                <span class="text-sm text-gray-400 line-through">{{ $product->price }} {{ __('SAR') }}</span>
                <span class="text-primary-600 font-bold text-lg">{{ $product->discount_price }} {{ __('SAR') }}</span>
                @else
                <span class="text-primary-600 font-bold text-lg">{{ $product->price }} {{ __('SAR') }}</span>
                @endif
            </div>
            ...
            <div class="flex space-x-2">
                <form action="{{ route('favorites.toggle', $product->id) }}" method="POST" class="ajax-wishlist-form">
                    @csrf
                    <button type="submit" class="p-2 bg-gray-50 rounded-full hover:bg-red-50 transition-colors {{ Auth::check() && Auth::user()->favorites->contains('product_id', $product->id) ? 'text-red-500' : 'text-gray-400 hover:text-red-500' }}" title="{{ __('Add to Favorites') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="{{ Auth::check() && Auth::user()->favorites->contains('product_id', $product->id) ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor">

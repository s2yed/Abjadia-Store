<template>
    <div class="bg-white rounded-lg shadow-sm p-6 max-w-2xl mx-auto">
        <h2 class="text-xl font-bold text-gray-800 mb-6">
            {{ isEditMode ? $t('edit_translator') : $t('add_translator') }}
        </h2>
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <div class="flex justify-end gap-3">
                <router-link to="/dashboard/translators" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200">{{ $t('cancel') }}</router-link>
...
export default {
    ...
    methods: {
        ...
        async fetchTranslator() {
            ...
            this.loading = true;
            try {
                const { data } = await axios.get(`/api/translators/${id}`);
                ...
                this.$router.push('/dashboard/translators');
            } finally {
                this.loading = false;
            }
        },
        async submitForm() {
            ...
            try {
                if (this.isEditMode) {
                    ...
                    await axios.post(`/api/translators/${this.$route.params.id}`, formData);
                } else {
                    await axios.post('/api/translators', formData);
                }
                this.$router.push('/dashboard/translators');
            } catch (err) {
                console.error(err);

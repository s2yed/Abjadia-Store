<template>
    <div class="bg-white rounded-lg shadow-sm p-6 max-w-2xl mx-auto">
        <h2 class="text-xl font-bold text-gray-800 mb-6">
            {{ isEditMode ? $t('edit_author') : $t('add_author') }}
        </h2>
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <div class="flex justify-end gap-3">
                <router-link to="/dashboard/authors" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200">{{ $t('cancel') }}</router-link>
    ...
    methods: {
        ...
        async fetchAuthor() {
            ...
            try {
                ...
                this.$router.push('/dashboard/authors');
            } finally {
                this.loading = false;
            }
        },
        async submitForm() {
            ...
            try {
                if (this.isEditMode) {
                    ...
                    await axios.post(`/api/authors/${this.$route.params.id}`, formData);
                } else {

<template>
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div v-if="totalPages > 1" class="mt-4 flex justify-between items-center text-sm text-gray-500">
            <button @click="fetchUsers(currentPage - 1)" :disabled="currentPage === 1" class="disabled:opacity-50">{{ $t('previous') }}</button>
            <span>{{ $t('page_of', { current: currentPage, total: totalPages }) }}</span>
            <button @click="fetchUsers(currentPage + 1)" :disabled="currentPage === totalPages" class="disabled:opacity-50">{{ $t('next') }}</button>
        </div>
